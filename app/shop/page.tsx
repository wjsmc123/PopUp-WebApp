...
export default function ShopPage() {
  ...
  useEffect(() => {
    if (!user) {
      hasFetchedProducts.current = null;
      router.push('/login');
      return;
    }

    if (hasFetchedProducts.current === user.eventId) {
      return;
    }

    hasFetchedProducts.current = user.eventId;
    const fetchProducts = async () => {
      const items = await getProducts(user.eventId);
      setProducts(items);
      setLoading(false);
    };
